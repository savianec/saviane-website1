...
interface WarningGraphicProps {
  ...
  color?: string;
  /**
   * When true (default), waits one frame after mount before running the
   * entrance sequence so animations are visible inside modals/portals.
   */
...
export function WarningGraphic({
  ...
  const [entranceReady, setEntranceReady] = React.useState(!deferEntrance);

  React.useEffect(() => {
    if (!deferEntrance) return;
    let r1 = 0;
    let r2 = 0;
    r1 = window.requestAnimationFrame(() => {
      r2 = window.requestAnimationFrame(() => setEntranceReady(true));
    });
    return () => {
  ...
  return (
    <motion.svg
      ...
      animate={
        !shouldAnimate || entranceReady ? "visible" : "hidden"
      }
    ...
    >
      <motion.g>
        ...
        <motion.g variants={containerVariants}>
          ...
          />
          <motion.path
            d="M57.3,57.531a.375.375,0,0,1-.321-.182L47.147,41.043H18.507l-7.71-7.71H7.66a.375.375,0,0,1,0-.75h3.448l7.709,7.71H47.571L57.623,56.962a.376.376,0,0,1-.127.515A.382.382,0,0,1,57.3,57.531Z"
            fill="none"
            stroke={color}
            strokeWidth="0.5"
            strokeLinecap="round"
